...
pub fn run() {
    ...
    println!("--------------------------");
    println!("Advent of Code 2022 Day 08");
    println!("--------------------------\n");
...
fn process_input_second(input: &str) -> usize {
    ...
    for y in 0..grid.len() {
        ...
        for x in 0..grid[0].len() {
            ...
            let taller_tree = trees
                .keys()
                .filter(|&key| key >= &grid[y][x].try_into().unwrap())
                .map(|key| trees[key].last().unwrap())
                .max();
            ...
            trees
                .entry(grid[y][x].try_into().unwrap())
                .and_modify(|positions| positions.push(x))
                .or_insert(vec![x]);
        }
    }
    ...
    for x in 0..(grid[0].len()) {
        ...
        for y in 0..grid.len() {
            ...
            trees
                .entry(grid[y][x].try_into().unwrap())
                .and_modify(|positions| positions.push(y))
                .or_insert(vec![y]);
        }
    }
